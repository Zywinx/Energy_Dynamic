...
import numpy as np
from wsi_core.slide_backend import open_wsi, read_region_rgb, get_properties, get_level_dimensions, get_level_downsamples
...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
        ...
        self.wsi = open_wsi(path)
        self.level_downsamples = self._assertLevelDownsamples()
        self.level_dim = get_level_dimensions(self.wsi)
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
        ...
        img = np.array(read_region_rgb(self.wsi, (0,0), seg_level, self.level_dim[seg_level]))
    ...
    def visWSI(self, vis_level=0, color = (0,255,0), hole_color = (0,0,255), annot_color=(255,0,0), 
        ...
        img = np.array(read_region_rgb(self.wsi, top_left, vis_level, region_size))
    ...
    def _getPatchGenerator(self, cont, cont_idx, patch_level, save_path, patch_size=256, step_size=256, custom_downsample=1,
        ...
        for y in range(start_y, stop_y, step_size_y):
            for x in range(start_x, stop_x, step_size_x):
                ...
                count+=1
                patch_PIL = read_region_rgb(self.wsi, (x,y), patch_level, (patch_size, patch_size))
                if custom_downsample > 1:
        ...
        for x_start in range(top_left[0], bot_right[0], block_size_x * int(downsample[0])):
            for y_start in range(top_left[1], bot_right[1], block_size_y * int(downsample[1])):
                ...
                if not blank_canvas:
                    ...
                    pt = (x_start, y_start)
                    canvas = np.array(read_region_rgb(self.wsi, pt, vis_level, blend_block_size))     
                else:
