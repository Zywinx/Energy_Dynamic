...
def _normalize_scores_array(scores, name='scores'):
...
def _normalize_coords_array(coords, scores=None, name='coords'):
    ...
    if coords.ndim == 1:
        if scores is not None:
            ...
            if n_scores is not None and coords.size == n_scores * 2:
                return coords.reshape(n_scores, 2).astype(np.int64)

        if coords.size % 2 == 0:
            return coords.reshape(-1, 2).astype(np.int64)

    raise ValueError(
        f"{name} cannot be normalized to shape (N, 2). "
        f"Current shape={coords.shape}, size={coords.size}"
    )
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(process_stack))):
        ...
        # wsi_kwargs is used later by compute_from_patches() when calc_heatmap=True.
        # blocky_wsi_kwargs above is for block-map feature extraction; this one uses
        # the overlap-aware step_size and current ROI bounds.
        wsi_kwargs = {
            'top_left': top_left,
            'bot_right': bot_right,
        ...
        for sample in samples:
            if not sample.get('sample', False):
                print('skip sampling {}'.format(sample.get('name', 'unnamed_sample')))
                continue
            ...
            # Top-patch sampling is intentionally disabled for the TOP1 representative heatmap smoke run.
            # Re-enable after heatmap generation is confirmed.
            continue
