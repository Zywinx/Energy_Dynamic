...
def _normalize_scores_for_heatmap(scores):
...
def _normalize_coords_for_heatmap(coords, scores=None):
    ...
    raise ValueError(
        f"coords cannot be normalized to (N, 2). shape={coords.shape}, size={coords.size}"
    )
...
def score2percentile(score, ref):
    # Robust scalar percentile conversion.
    # scipy.stats.percentileofscore can return an array if score/ref has an
    # unexpected shape. Heatmap attention A[score_idx, 0] needs a scalar.
    ref = np.asarray(ref).reshape(-1)
    score = float(np.asarray(score).reshape(-1)[0])
    percentile = percentileofscore(ref, score)
    return float(np.asarray(percentile).reshape(-1)[0])
        ...
        with torch.inference_mode():
            ...
            if attn_save_path is not None:
                ...
                if ref_scores is not None:
                    ref_scores = np.asarray(ref_scores).reshape(-1)
                    for score_idx in range(len(A)):
                        A[score_idx, 0] = score2percentile(A[score_idx, 0], ref_scores)
